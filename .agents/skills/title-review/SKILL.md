---
name: title-review
description: 從 revswarm DB 取 100 筆 verified IS NULL 的 success，逐條閱讀 raw_title 並審核：撐得起日期的寫進 data/title-review-codex.csv 並蓋 verified='codex'，撐不起的打回 undone 並改排 yahoo 佇列。當使用者說「審 title」「讀 100 筆 title」「繼續審核」時使用。
---
...
### Yahoo 反覆錯誤時交給 Google

審核排程要留意同一任務的 `fail_count`、`engine` 與 worker 回報，不要讓 Yahoo
反覆用同一條失敗路徑重試。若明確看到同一筆由 Yahoo 反覆回報 `failed`（或該筆
已累積多次 Yahoo 失敗且仍在 `engine='yahoo'` 的 undone 佇列），用帶 token 的
`POST /admin/requeue`，以該筆當下的 `stock_id/roc_year/roc_month/announce_date`
作樂觀鎖，並指定 `"engine":"google"`：

```bash
python3 - <<'PY'
import json, os, urllib.request
items = [{"stock_id": "...", "roc_year": 0, "roc_month": 0,
          "date": "YYYY-MM-DD"}]
req = urllib.request.Request(
    "http://127.0.0.1:8000/admin/requeue",
    data=json.dumps({"items": items, "engine": "google"}).encode(),
    method="POST",
...
`rate_limited` 不是 Yahoo 內容錯誤，不因單純限流就轉 Google；先讓它按原佇列
退避重試。若 server 的自動升級鏈已把 Yahoo 的 `failed` 改成 `engine='google'`，
不要重複 requeue。轉交後要確認該筆的 `state/engine` 已落成 Google 佇列；若
`/admin/requeue` 回 fatal 錯誤，停止排程並回報，不可直接寫 DB。

## 2. URL 補強證據

逐筆讀完 `raw_title` 與任務欄位後，若該列有 `url` 且標題被截斷、缺少金額／日期，或公司與內容需要確認，可以開啟 URL 閱讀完整頁面。URL 是補充證據，不是「有 URL 就可信」的捷徑。

- 先確認頁面的文章標題、正文或表格，以及頁面自己的發布／事實發生日；優先使用公司、MOPS、MoneyDJ、中央社等頁面上的直接內容。
- 必須逐筆把頁面內容與 `stock_id/name`、ROC 年月、`announce_date`、`revenue`、`yoy` 對照。頁面若只顯示搜尋結果、跳轉失敗、無法確認公司／年月／營收，不能補足證據。
- 頁面自己的日期可以補足截斷的 SERP 日期；Goodinfo 的彙整「發布時間」不能單獨當作公告時戳，仍要看獨立的營收內容與金額。
- URL 頁面與 `raw_title` 或 DB 有矛盾時，停下來個案判斷；不要用其中一個吻合欄位掩蓋另一個矛盾。無法排除錯公司、錯年月、非營收或金額量級錯誤時打回。
- 若 URL 開啟失敗，回到標題本身判斷；標題不足以支撐就打回，不要把網路錯誤當成可信證據。
- URL 佐證的具體事實要寫進該筆專屬 `note`（例如頁面日期、仟元表格數字與 DB 的對應）；CSV schema 不新增 URL 欄位。

開啟頁面可用瀏覽工具或唯讀 HTTP 讀取；不得用正則、分類器、啟發式批次篩選代替逐筆閱讀。
...
⚠️ **Goodinfo 陷阱還有第二種變體：時戳是真的（每家公司、每月都不同），但金額本身離譜**。
實測踩過：遠傳 114/9、台聯電 114/9 與 114/12、前鼎 114/11，都是
`合併營業收入淨額應為X仟元 - 發布時間:YYYY/MM/DD HH:MM:SS` 這種格式，時戳每筆都不同、
不是彙整頁的循環論證，但金額比該檔正常月營收規模大 10～100 倍。跟上面「同一秒鐘」那種
一眼假不同，這種要靠**金額量級核對**才抓得到——時戳過關不代表金額過關，兩件事要分開查。
...
⚠️ **MOPS 仟元表格的「去年同期」欄位可以拿來交叉核對**：表格裡除了「本月」還會附
「去年同期」數字，如果去年同期那個月已經蓋過章，直接比對兩者是否一致——實測事欣科
114/10 表格「去年同期284,118」對上已蓋章 113/10 的 `revenue=284000000`，幾乎完全吻合，
等於白拿一次獨立驗證。極端 yoy% 的月份也可以反推：`本年營收 / (1 + yoy/100)` 算出隱含的
去年同期基期，再對照已蓋章的實際數字，用來分辨「真的成長」還是「資料本身有問題」。
...
| `2409 友達 111/12`：標題「12月自行結算合併營收為180億元」，但 DB `revenue` 卻等於標題另一段「第四季合併營收526.5億元」 | ⚠️ 單月 vs 季度／累計數字混淆：`revenue` 欄位被填成標題裡另一個期別的數字，不是同一個月——要看標題裡「這個月」對應的那個金額，不能看 `revenue` 是否等於標題裡*某個*數字就放行 |
| `2881 富邦金 111/12`：URL 片段公司名寫的是「台北富邦銀行」 | ⚠️ 控股公司 vs 子公司抓錯：任務要的是富邦金（控股），抓到的是旗下銀行子公司的揭露，不是同一個申報主體——跟聯上/聯上發那種「抓到別家公司」同類但層級不同（母子公司而非同業） |
| `2882 國泰金`、`2885 元大金`、`2892 第一金`（111/12）：標題寫「年獲利」「年每股盈餘」 | ⚠️ 金控股的獲利/EPS vs 營收混淆：金控業常把自結獲利當主要揭露指標，但這個任務要的是月營收數字，不是獲利或每股盈餘——公司名年月都對，但任務類型不符，跟一般製造業的 EPS 自結公告是同一類判準 |
...
| `News 科技新報 …/primax-`（task 的 `name` 欄位是「致伸」） | 公司英文品牌名（Primax Electronics）而非中文名，跟簡稱/改名是同一類判準，只是換了語言——用股票代碼核對是同一檔就好 |
| URL slug「示其-驊」對到公司「祺驊」；標題「御頂」對到公司「御嵿」 | 字型渲染/OCR 把字拆成部首（祺→示+其）或换成形近字（嵿→頂）——用股票代碼核對是同一檔，非抓錯公司 |
| `研勤 111/12`：SERP 片段開頭顯示「2月營收0.58億元」，但同段還有「1—12月達3.80億元」累計字樣 | 截斷把「12月」的前導「1」吃掉、變成看起來像「2月」——要看累計金額的月數是否對得上（1~12月＝12個月），不能只看片段最前面那個數字 |
| `2855 統一證 111/12`：標題寫「損益」不是「營收」，但格式是`發言人：安芝立說明：1.事實發生日:112/01/09` | 證券商月度自結揭露主指標本就是損益（P&L）而非傳統營收，這裡雖字面是「損益」但套用的是硬佐證的具名發言人＋事實發生日格式，屬業別慣例的例外，非任務類型不符 |
...
追加進 `data/title-review-codex.csv`（欄位 `stock_id,roc_year,roc_month,announce_date,verdict,note`），
`verdict=codex`，`note` 寫**這一筆**的理由。然後：

```bash
python3 -m stamp_verified --from codex --review data/title-review-codex.csv --server http://127.0.0.1:8000
```
...
- 出現沒見過的錯法就明講，那比數字有價值
- `git status` 確認只動了 `data/title-review-codex.csv`
