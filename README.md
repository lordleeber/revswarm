...
**撞到人工驗證時的處理（`/sorry/` 或頁面出現「異常流量」）：第一筆就立刻停批**，
不再打第二種年份、也不再取下一筆任務 —— 畫面就停在驗證頁。
之後每 `--captcha-poll`（預設 5s）秒只「讀」一次當前頁判斷是否已解除，**不導覽**，
所以你在上面輸入不會被打斷；**解掉的瞬間就自動接續**，不必等滿 `--block-sleep`。

> ⚠️ 這裡踩過一次坑：原本驗證碼也套 `--rl-threshold`（要連續 3 筆才停批），
> 但期間每一筆的 `page.goto` 都會把正在解的驗證頁蓋掉，實際上**根本解不完**。
> 現在驗證碼不套那個門檻 —— Google 要求驗證代表整個 IP 被攔，繼續打也只是拿到同一頁。

`--rl-threshold` 仍然管「非驗證碼」的連續失敗（導覽逾時、頁面不是 SERP…）：
超過門檻就提早收批並長睡 `--block-sleep`。那種情況畫面通常是正常的、沒有東西可解，
所以刻意保留無條件長睡，避免連續失敗時繼續燒自己的 IP。
